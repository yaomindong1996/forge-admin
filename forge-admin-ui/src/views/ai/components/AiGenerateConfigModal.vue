<template>
  ...
  >
    ...
    <n-alert type="info" style="margin-bottom: 16px;">
      <template #header>快速生成 vs 对话式生成</template>
      <div style="font-size: 13px; line-height: 1.6;">
        <p><strong>快速生成（当前）</strong>：适合简单场景，输入描述或表名后一键生成配置，生成后可直接使用。</p>
        <p><strong>对话式生成</strong>：适合复杂场景，支持多轮对话迭代优化配置，实时预览和编辑。 
          <n-button text type="primary" size="small" @click="goToGenerator">前往对话式生成 →</n-button>
        </p>
      </div>
    </n-alert>
      ...
      <n-form ref="formRef" :model="formData" label-placement="left" label-width="100">
        ...
        <n-form-item label="configKey" path="configKey">
          <n-input v-model:value="formData.configKey" placeholder="小写字母开头，仅含小写字母+数字+下划线">
            <template #suffix>
              <n-button text size="tiny" type="primary" @click="autoGenerateKey">自动生成</n-button>
            </template>
          </n-input>
        </n-form-item>
...
function autoGenerateKey() {
  ...
  if (formData.value.tableName) {
    ...
    const keywordMap = {
      '管理': 'manage', '系统': 'system', '配置': 'config', '列表': 'list',
      '用户': 'user', '角色': 'role', '菜单': 'menu', '部门': 'department',
      '员工': 'employee', '产品': 'product', '订单': 'order', '客户': 'customer',
    }
    let words = []
    for (const [cn, en] of Object.entries(keywordMap)) {
      if (text.includes(cn)) words.push(en)
    }
    if (words.length > 0) key = words.slice(0, 3).join('_')
  }
  if (!key) key = 'page_' + Math.random().toString(36).substr(2, 4)

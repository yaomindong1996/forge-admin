...
/**
 * EmlEmailDocumentReader - Used to read .eml email files and convert them to Document format This
 * reader supports: 1. Reading email content (both HTML and plain text) 2. Extracting email metadata
 * (subject, from, to, date, etc.) 3. Processing email attachments
 *
 * @author xiadong
 * @since 2024-01-19
 */
public class EmlEmailDocumentReader implements DocumentReader {
  ...
  public List<Document> get() {
    try (InputStream inputStream = emailResource.getInputStream()) {
      ...
      for (EmailElement element : headerElements) {
        ...
        } else if (element instanceof ReceivedInfo info) {
  ...
  private void processMessageContent(
      Part part, Map<String, Object> metadata, List<Document> documents) throws Exception {

...
- [StreamGenerateRequest.java](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/dto/StreamGenerateRequest.java)
...
**更新** 新增AI驱动的CRUD配置生成器，采用"AI代理-流式生成-可视化编辑"的全新架构模式，以及页面模板系统支持不同生成策略。
...
H["AiPageTemplateService<br/>页面模板服务"]
end
subgraph "模板资源"
T1["entity.java.vm"]
T2["mapper.java.vm"]
...
- [CrudGeneratorStreamService.java:1-326](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/service/CrudGeneratorStreamService.java#L1-L326)
...
## 架构总览
代码生成器采用"配置驱动 + 模板渲染"的传统架构，**新增**AI驱动的CRUD配置生成器采用"AI代理-流式生成-可视化编辑"的现代化架构模式，以及页面模板系统支持不同生成策略：
...
participant TemplateSvc as "AiPageTemplateService"
...
StreamSvc->>TemplateSvc : 加载页面模板约束
loop 多阶段生成
...
- [AiPageTemplateService.java:42-54](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/service/AiPageTemplateService.java#L42-L54)
...
E["AiPageTemplateService<br/>页面模板服务"]
end
subgraph "前端界面"
F["crud-generator.vue<br/>主界面"]
G["SchemaFieldEditor.vue<br/>字段编辑器"]
H["ApiConfigEditor.vue<br/>API配置编辑器"]
I["useCrudGenerator.js<br/>状态管理"]
end
subgraph "数据存储"
J["ai_crud_config<br/>配置存储"]
K["ai_session_messages<br/>会话存储"]
L["gen_table<br/>表结构存储"]
M["ai_page_template<br/>页面模板存储"]
end
A --> C
A --> D
B --> C
...
- [AiPageTemplateService.java:51-54](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/service/AiPageTemplateService.java#L51-L54)
...
### 模板架构
页面模板系统为AI生成器提供不同的生成策略和约束条件：

```mermaid
graph TB
subgraph "页面模板系统"
A["AiPageTemplate<br/>页面模板实体"]
B["AiPageTemplateService<br/>模板服务"]
C["模板类型<br/>TEMPLATE/AI"]
D["系统提示<br/>systemPrompt"]
E["Schema约束<br/>schemaHint"]
F["默认配置<br/>defaultConfig"]
end
subgraph "模板策略"
G["TEMPLATE<br/>Velocity模板生成"]
H["AI<br/>大模型生成"]
end
A --> B
B --> G
...
**图表来源**
- [AiPageTemplate.java:14-52](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/domain/entity/AiPageTemplate.java#L14-L52)
- [AiPageTemplateService.java:27-54](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/service/AiPageTemplateService.java#L27-L54)

### 模板类型
系统支持两种生成策略：

- **TEMPLATE（默认）**：使用Velocity模板生成，适用于简单CRUD等结构化组件
- **AI（大模型）**：使用大模型生成，适用于复杂组件（主从表、看板、工作流等）

### 模板约束
页面模板提供以下约束和配置：

- **系统提示**：模板专属的AI系统提示补充
- **Schema约束**：模板可用的字段约束（JSON格式）
- **默认配置**：模板的默认配置值（JSON格式）
- **启用状态**：模板的启用/停用状态

**章节来源**
- [AiPageTemplate.java:29-52](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/domain/entity/AiPageTemplate.java#L29-L52)
- [AiPageTemplateService.java:109-134](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/service/AiPageTemplateService.java#L109-L134)
...
- **模板服务依赖**：AiPageTemplateMapper、ObjectMapper
...
Stream --> SchemaGen
Stream --> ObjectMapper
Template["AiPageTemplateService"] --> TemplateMapper["AiPageTemplateMapper"]
Template --> ObjectMapper
...
- [AiCrudConfigGenerateService.java:24-27](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/service/AiCrudConfigGenerateService.java#L24-L27)
- [CrudGeneratorStreamService.java:26-29](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/service/CrudGeneratorStreamService.java#L26-L29)
- [AiPageTemplateService.java:25](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/service/AiPageTemplateService.java#L25)
...
- **页面模板列表接口**：`GET /ai/page-template/list-enabled`
...
### 页面模板配置
- **模板类型**：TEMPLATE（Velocity模板）和 AI（大模型）
- **系统提示**：模板专属的AI提示补充
- **Schema约束**：模板可用字段约束
- **默认配置**：模板的默认配置值
- **启用状态**：模板的启用/停用状态
...
**章节来源**
- [AiPageTemplate.java:17-52](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/domain/entity/AiPageTemplate.java#L17-L52)
- [AiPageTemplateService.java:109-134](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/service/AiPageTemplateService.java#L109-L134)
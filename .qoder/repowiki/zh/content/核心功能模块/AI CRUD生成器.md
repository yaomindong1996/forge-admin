...
- [AiPageTemplateDTO.java](file://forge/forge-framework/forge-plugin-parent/forge-plugin-generator/src/main/java/com/mdframe/forge/plugin/generator/dto/AiPageTemplateDTO.java)
</cite>

## 更新摘要
**变更内容**
- 新增页面模板管理功能，支持标准CRUD和树形CRUD模板
- 增强模板选择机制，支持动态模板加载和配置
- 完善页面渲染流程，支持模板化页面生成
- 新增模板管理API和后端服务支持
...
**重大增强**：系统现已支持页面模板管理功能，包括标准CRUD模板和树形CRUD模板，为不同数据结构提供专门的页面布局和交互方式。新增的模板管理系统允许用户创建、管理和自定义页面模板，每个模板都可配置专属的AI提示词约束和默认配置。
...
H[ApiConfigEditor]
I[DictConfigPanel]
J[DesensitizeConfigPanel]
K[EncryptConfigPanel]
L[SimpleCrudTemplate]
M[TreeCrudTemplate]
N[PageTemplateCard]
O[TemplateCatalog]
end
subgraph "状态管理层"
P[useCrudGenerator组合式函数]
Q[SSE流式生成器]
R[模板选择状态]
S[模板列表管理]
end
...
W[AI页面模板API]
X[模板管理服务]
Y[AiPageTemplateController]
Z[AiPageTemplateService]
end
A --> B
A --> C
A --> D
...
- [catalog/index.js:1-55](file://forge-admin-ui/src/catalog/index.js#L1-L55)
...
+templateList : Ref~Array~
+loadTemplateList() Promise
+getTemplateById(id) AiPageTemplate
+getTemplateByKey(key) AiPageTemplate
+createTemplate(dto) Promise
+updateTemplate(dto) Promise
+deleteTemplate(id) Promise
}
UseCrudGenerator --> StreamGenerate : uses
UseCrudGenerator --> TemplateManager : manages
```
...
- [page-template.js:1-41](file://forge-admin-ui/src/api/page-template.js#L1-L41)
...
6. **JSON验证**：模板配置的JSON格式进行严格验证，避免无效数据
...
| 模板验证失败 | 模板创建/更新失败 | JSON格式不正确 | 检查模板配置的JSON格式 |
| 模板列表为空 | 模板选择下拉框无选项 | 模板服务异常 | 检查模板服务状态 |
...
6. **API测试**：使用Postman或curl测试模板管理API

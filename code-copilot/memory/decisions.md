...
## 41. 应用页面设计与业务对象设计使用双工作台语义

**记录日期**: 2026-07-26

低代码应用不向业务用户暴露“新版/旧版设计器”的技术概念。应用工作台中的两条主线必须使用业务语义区分：

- **应用页面设计**：维护最终用户看到的页面、菜单、组件和页面间跳转；入口固定打开应用 runtime 的 `edit=1`。
- **业务对象设计**：维护字段、表单、列表、详情、数据规则和流程；入口固定打开既有 `BusinessObjectDesigner`，并保留 `returnTo` 返回页面设计上下文。

对象绑定的 CRUD 只能引用业务对象设计产物，不能在应用页面中复制一套字段或对象表单；应用内表单资产仅用于独立录入或内容页，统一称为“页面资源 / 页面表单资产”。应用运行、草稿预览和页面设计必须是三个独立入口，禁止再用含糊的“进入应用”同时表达它们。

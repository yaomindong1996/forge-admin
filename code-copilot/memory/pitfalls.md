...
## 113. pnpm 仅更新锁文件也可能移动现有依赖目录

**发现日期**: 2026-07-13

**问题描述**:
在已有 `node_modules` 的前端工作区执行 `pnpm install --lockfile-only` 后，pnpm 可能把当前依赖移动到 `node_modules/.ignored`，但不会恢复完整可执行链接。随后执行 `pnpm exec vitest` 会报：
...
**解决方案**:
- 不要把 `pnpm install --lockfile-only` 当成无副作用的依赖恢复命令。
- 只需要更新锁文件时，先确认当前工作区是否依赖现有 `node_modules` 做后续验证，并记录命令可能改变依赖布局。
- 出现 `.ignored` 和命令缺失后，应按项目 Node 版本执行完整依赖恢复，再运行测试；不能把“命令未找到”记录成测试失败或通过。
- 用户明确自行验证或不授权修复依赖时，保持验证项 `not-run/pending`，不要继续修改依赖环境。

**影响范围**:
- `forge-admin-ui` 的 Vitest、Vite、ESLint 和生产构建命令。
- 使用 pnpm 且复用已有 `node_modules` 的阶段验证流程。

## 114. Vite 的 global 兼容映射不能指向 window

**发现日期**: 2026-07-14

**问题描述**:
前端主页面功能正常，但 module Worker 创建后立即报错：

```text
Uncaught ReferenceError: window is not defined (env.mjs:8:139)
```

此时 Worker 尚未发送 READY，用户扩展脚本也未开始执行。

**根本原因**:
`vite.config.js` 使用 `define.global = 'window'` 为浏览器依赖提供 Node 风格全局对象。Vite 会把该 define 注入 Worker 的 `env.mjs`；Web Worker 没有 `window`，所以模块初始化阶段直接抛出 `ReferenceError`。

**解决方案**:
- 将兼容映射改为 `define.global = 'globalThis'`，同时适配页面主线程和 Web Worker。
- 修改 `vite.config.js` 后必须停止并重新启动开发服务；只刷新浏览器不能保证旧配置和依赖图被替换。
- 若重启后仍出现旧错误，再清理 `node_modules/.vite` 预构建缓存并强制重启，不能把该启动异常误判为用户扩展脚本错误。

**影响范围**:
- 使用 Vite `define` 注入全局对象的 module Worker。
- 客户端扩展沙箱及其它通过 `new Worker(..., { type: 'module' })` 启动的前端能力。

## 115. 字段名称推断不能覆盖数据库导入或用户明确选择的字段类型

**发现日期**: 2026-07-15

**问题描述**:
数据库导入字段已经明确为 `TEXT`、`SWITCH`、`NUMBER` 等类型，但保存业务对象设计器时，只要中文字段名以“状态”或“类型”结尾，仍被名称规则强制改为 `DICT`，随后因没有 `dictType` 抛出“字典字段必须配置字典类型”。

**解决方案**:
- 字段类型归一化必须优先采用非空显式 `fieldType`；名称推断只能用于字段类型为空的旧数据或自动创建场景。
- 显式类型别名可以归一化，未知显式类型安全降级为 `TEXT`，但不能再进入中文名称推断。
- 真正的 `DICT/SELECT/RADIO/CHECKBOX/MULTI_SELECT` 字段仍要求字典类型或有效静态选项，不能为绕过误判而放宽字典治理。
- 校验异常应携带字段名称或编码，便于从设计器一次定位问题字段。

**影响范围**:
- 数据库表导入后的业务对象保存。
- 关系设计器重新提交全量字段的保存链路。
- 所有同时存在显式字段类型和中文名称智能推断的字段协议转换。

## 116. 设计预览不能复用发布阶段缓存的运行配置

**发现日期**: 2026-07-15

**问题描述**:
主子表模板已经创建对象和关系，但如果设计预览直接读取 `AiCrudConfig` 中上次发布生成的 `searchSchema/columnsSchema/editSchema/apiConfig`，新关系、子对象字段变更和表单草稿都不会进入真实 CRUD 页面。用户会误以为必须逐个发布对象才能预览。

**解决方案**:
- `designPreview` 先按 `configKey` 定位业务对象，根据当前关系表和子对象最新草稿刷新主对象 `modelRefs`。
- 草稿预览只要存在 `modelSchema + pageSchema` 就强制调用运行配置构建器，不命中发布期运行 Schema 缓存。
- 普通运行继续读取不可变发布版本，不能因修复预览而放宽发布门禁。
- 应用发布前刷新 PRIMARY 聚合图即可，避免对所有对象重复水合关系造成 N+1 查询和无意义草稿版本增长。

**影响范围**:
- 主子表、左树右表和复用已发布对象后的新增关系。
- 子对象字段变更后的主对象明细预览。
- 同时保存设计草稿和发布运行快照的低代码渲染链路。

## 117. 主子表代码生成不能因派生配置为空静默退化为单表

**发现日期**: 2026-07-15

**问题描述**:
主子表 Service 模板已经包含明细查询、事务新增、替换更新和级联清理，但生成策略只根据 `options.masterDetailConfig.children` 是否非空决定是否进入模板分支。旧草稿、局部保存或派生配置字段丢失时，生成会表面成功，实际却输出普通单表 `ServiceImpl`，用户只能在下载后才发现子表逻辑缺失。

**解决方案**:
- 主子表生成优先读取派生配置，同时从权威页面协议 `pageSchema.modelRefs` 的 `CHILD_LIST/DETAIL/ONE_TO_MANY` 关系补齐子对象、子表外键和主表关联字段。
- 兼容关系存储在子引用指向主对象，以及主引用指向子对象两种方向，并按方向正确交换主外键。
- `layoutType=master-detail-crud` 但两处均无法解析有效子关系时必须失败关闭，返回可定位的业务异常；禁止静默设置 `isMasterDetailLayout=false`。
- 契约检查同时覆盖兜底方法、失败文案和模板中的明细查询/保存方法，避免只证明模板存在而未证明生成分支可达。

**影响范围**:
- 应用级完整代码预览和 ZIP 下载。
- 对象级 Velocity 主子表代码生成。
- 任何同时保存权威页面协议与派生运行 options 的生成链路。

## 118. Velocity 模板上下文不能使用私有元数据类型

**发现日期**: 2026-07-15

**问题描述**:
模板上下文传入 `List<private static RelatedTableMeta>` 时，Velocity 可以执行 `#foreach`，但无法通过反射访问私有声明类型上的 Lombok getter。生成不会必然抛出异常，而是把 `${table.className}`、`${child.className}` 等引用原样写入 Java 文件，导致预览看似有文件、下载后却无法编译。仅扫描模板源码中是否包含主子方法无法发现这个问题。

**解决方案**:
- 所有需要由 Velocity 读取属性的上下文类型必须是公开类型，或在入参前转换为公开 Map/DTO；公开 getter 不能弥补声明类本身不可访问。
- 契约测试必须至少实际渲染一次关联对象模板，并断言具体类名、Mapper 名和主子查询/保存/清理方法已经展开。
- Java 产物写入文件集合前扫描未解析的 `$variable.property` 和 `${...}`；发现残留立即失败关闭。
- 模板渲染异常不得只记日志后跳过文件，否则“完整代码包”会静默缺少 Service、Mapper 或 DTO。

**影响范围**:
- Velocity 主子表、左树右表和其它集合/嵌套对象模板。
- 应用级代码预览、ZIP 下载以及任何以文件集合成功返回作为完成标志的生成链路。

## 119. 重编译运行配置时布局类型也必须由最新页面协议覆盖

**发现日期**: 2026-07-15

**问题描述**:
代码生成已经根据最新 `pageSchema` 重建出主子表关系和 `masterDetailConfig`，但如果 `layoutType` 使用 `defaultIfBlank(旧值, 新值)`，历史 `simple-crud` 会继续优先。最终表现为子表 Mapper 已注入主 Service，`isMasterDetailLayout` 却为 false，主子查询、事务保存和清理分支全部不生成。

**解决方案**:
- 只要模型和页面协议参与本次重编译，`runtimeConfig.layoutType` 必须优先覆盖旧 `AiCrudConfig.layoutType`。
- 契约测试同时断言旧值优先写法不存在，不能只检查 search/list/edit Schema 已覆盖。
- 出现“有关联依赖注入但无布局逻辑”时，要同时核对关系集合和布局分支条件，不能只检查模板或关系解析其中一侧。

**影响范围**:
- 单表切换主子表、左树右表后的草稿代码生成。
- 旧发布配置或旧运行派生字段仍存在的应用级代码预览和下载。

## 120. 协议运行代码包不能只在应用组装层改 generated 键

**发现日期**: 2026-07-15

> **历史方案已废止**：Phase 12 已取消下载后端 classpath 协议注册和动态 CRUD 委托。本条只用于理解旧实现，不再作为当前修复建议。

**问题描述**:
应用级组装器已经把配置键改为 `generated_*`，但旧 configKey 下载和低代码应用直接下载仍可能把普通数据库配置键写入 `META-INF/forge-lowcode`。启动注册器为避免覆盖平台配置会拒绝该资源；如果放宽注册器校验，又会产生 classpath 与数据库同键抢占。另一个常见遗漏是只投影 `/page`，导致左树右表回退列表接口加载左树；异步导出任务也可能继续轮询 `/ai/crud/...`。

**解决方案**:
- 生成键派生和业务 API 投影必须位于所有入口都会经过的公共生成策略，而不是只放在应用组装服务。
- 输出目录键与运行配置键分离：旧入口可以保持原页面目录，Controller 和 classpath 资源只能使用隔离运行键。
- 业务 API 必须包含 `/tree`、POST-safe CRUD、导入导出以及异步导出任务列表/详情；共享前端优先读取这些业务端点。
- `/ai/crud-page/<sourceKey>` 是前端页面跳转，必须改写为生成页面路由，不能误改成后端 `apiBase`。
- 无法确定目标代码包归属的其它 `/ai/crud/` 引用必须失败关闭，禁止用字符串前缀替换生成一个不存在的业务端点。

**影响范围**:
- 应用管理、访问入口、低代码应用和旧 CRUD 配置下载。
- 左树右表、异步导出、跨页面动作和 classpath 内嵌协议注册。

## 121. 下载 Controller 直接委托动态 CRUD 会封死正常二次开发

**发现日期**: 2026-07-15

**问题描述**:
生成包虽然包含 Entity、Mapper、Service 和 XML，但 Controller 实际全部调用 `DynamicCrudService` 时，这些静态分层只是旁路脚手架。用户修改 Mapper XML、在生成 Service 增加复杂事务或新增 Manager 都不会进入真实接口；导入导出如果调用 `DynamicCrudExcelService`，还会间接回到相同动态链路。

**解决方案**:
- 在线运行和下载源码分轨：在线保留动态 CRUD，下载 Controller 只调用生成的类型化 Service。
- ServiceImpl 继承 MyBatis-Plus `ServiceImpl`；基础写操作使用 MP，查询 SQL 写 Mapper XML，主子和导入事务放 Service。
- 每个主对象生成 default-method around 扩展接口；用户实现文件不进入生成集合，可完整替换查询/写入或增强默认逻辑。
- 文件所有权清单区分 `GENERATED`、`CREATE_ONCE_SAMPLE` 和用户目录；不要用“重新下载后人工避免覆盖”代替生成器边界。
- 完整协议快照继续保留，但 coverage 必须明确静态已编译和需要扩展的能力，不能用 JSON passthrough 冒充后端实现。

**影响范围**:
- 应用管理、访问入口、低代码应用和旧配置键的代码预览/下载。
- 单表、左树右表、主子表、复杂查询、复杂新增及导入导出二次开发。

## 122. GET 下载参数会丢失“显式空表前缀列表”语义

**发现日期**: 2026-07-15

**问题描述**:
代码预览和 ZIP 下载使用 GET 参数时，通用数组序列化通常会省略空数组。用户把 `stripTablePrefixes` 清空是明确要求“保留完整物理表名”，但参数一旦被省略，后端会误判为未配置并重新使用默认前缀，造成预览、保存设置和下载文件类名不一致。

**解决方案**:
- 保存接口继续用 JSON 数组；预览和下载把前缀列表统一序列化为逗号字符串。
- 对 `stripTablePrefixes` 特别保留空值参数，后端将空字符串/空集合规范化为空列表，不能回退默认值。
- 请求签名使用规范化数组，任一前缀顺序或空列表变化都使旧预览失效。

**影响范围**:
- 所有通过 GET 传递可清空集合的代码预览和文件下载接口。
- 应用级、访问入口级低代码代码包设置。

## 123. 配置分组转换后必须先持久化再刷新运行属性

**发现日期**: 2026-07-15

**问题描述**:
配置中心把 JSON 保存到 `sys_config_group` 后，仅调用 `ConfigConverter` 生成属性 Map 并触发 `ConfigRefresher`，不代表配置已经同步。刷新器会重新读取 `sys_config`；如果同步服务没有先写入该表，旧值会覆盖刚转换的新值，最终表现为页面提示保存成功，但 `@RefreshScope` 中的加解密等配置仍不生效。

**解决方案**:
- `ConfigSyncService` 必须先按默认租户 `tenant_id=1` 把转换结果 upsert 到 `sys_config`，再调用 `ConfigRefresher.refresh(...)`。
- 配置保存接口必须检查“分组保存 + 属性表同步 + 运行刷新”的组合结果，任一步失败都不能返回成功。
- 运行态开关切换接口应固定明文，避免关闭或重新开启时由新旧加密状态互相锁死。
- 浏览器需要的运行配置单独提供匿名、安全裁剪端点，禁止返回对称密钥、RSA 私钥等密钥材料；加载失败保持加密默认开启。

**影响范围**:
- `sys_config_group` 与 `sys_config` 双表配置中心。
- `@RefreshScope` 加解密、认证、日志、水印及登录配置。
- 需要在浏览器启动阶段读取的安全运行开关。

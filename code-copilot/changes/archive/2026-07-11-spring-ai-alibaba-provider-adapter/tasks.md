...
- **涉及文件**：
  ...
  - `code-copilot/changes/archive/2026-07-11-spring-ai-alibaba-provider-adapter/test-spec.md` — 回填本轮增量；
  - `code-copilot/changes/archive/2026-07-11-spring-ai-alibaba-provider-adapter/execution-log.md` — 记录真实命令和结果。
- **实施步骤**：
...
- **涉及文件**：
  ...
  - `code-copilot/changes/archive/2026-07-11-spring-ai-alibaba-provider-adapter/spec.md` — 更新执行日志和审查结论；
  - `code-copilot/changes/archive/2026-07-11-spring-ai-alibaba-provider-adapter/execution-log.md` — 记录收尾验证；
...
## Task 9: 收敛默认模型权威来源与安全诊断

- **目标**：删除跨供应商模型兜底，确保已保存供应商只使用 `ai_model` 中启用且默认的模型，并安全记录供应商状态码/错误码。
- **涉及文件**：
  - `forge-plugin-ai/src/main/java/com/mdframe/forge/plugin/ai/model/mapper/AiModelMapper.java`；
  - `forge-plugin-ai/src/main/resources/mapper/AiModelMapper.xml`；
  - `forge-plugin-ai/src/main/java/com/mdframe/forge/plugin/ai/model/service/AiModelService.java`；
  - `forge-plugin-ai/src/main/java/com/mdframe/forge/plugin/ai/client/AiInvocationResolver.java`；
  - `forge-plugin-ai/src/main/java/com/mdframe/forge/plugin/ai/provider/service/AiProviderService.java`；
  - `forge-plugin-ai/src/main/java/com/mdframe/forge/plugin/ai/provider/support/AiProviderFailureDiagnostics.java`；
  - 对应 `AiModelServiceTest`、`AiInvocationResolverTest`、`AiProviderServiceTest`、`AiProviderFailureDiagnosticsTest`。
- **实施步骤**：
  - [x] 先写失败测试，证明已保存连接测试忽略 `ai_provider.default_model` 并读取权威模型；
  - [x] 先写失败测试，证明没有启用默认模型时返回“请为供应商设置默认模型”且不创建 `ChatModel`；
  - [x] 在 Mapper XML 增加按供应商查询启用默认模型的单行查询，显式过滤 `status/is_default/del_flag`；
  - [x] `AiInvocationResolver` 保留显式模型和 Agent 模型优先级，默认分支改用 `AiModelService`；
  - [x] 删除 `AiProviderService` 与 `AiInvocationResolver` 中的 `gpt-3.5-turbo`；
  - [x] 新增只提取并白名单化 `httpStatus/errorCode` 的诊断工具，日志禁止输出原始异常 message；
  - [x] 运行四个目标测试类并用 `rg` 确认生产代码无通用 `gpt-3.5-turbo`。
- **验收标准**：无默认模型时零网络请求；正确默认模型进入 Adapter；日志可见 400/invalid_request_error 且不泄露响应正文或密钥。

## Task 10: 重构供应商与模型管理工作区

- **目标**：修复分页、上传和响应式缺陷，将页面重构为清晰的供应商—模型主从工作区。
- **涉及文件**：
  - `forge-admin-ui/src/views/ai/provider-model.vue`。
- **实施步骤**：
  - [x] 供应商分页增加 `pageSize/itemCount/show-size-picker`，查询统一为显式提交；
  - [x] 模型表增加独立服务端分页、每页条数和快速跳页，模型总数使用 `itemCount`；
  - [x] 改为参考图式左右主从布局和“基础配置 / 模型管理”页签，并提供窄屏上下布局；
  - [x] 供应商使用原生 button 列表项，重复点击保持选中，提供显式关闭入口；
  - [x] 修正两处上传 `accept`、两类弹窗宽度和 API Key 编辑提示；
  - [x] 统一供应商/模型操作为语义化文字按钮，补模型设默认入口；
  - [x] 连接测试弹窗增加成功/失败状态块和明确文案；
  - [x] 检查暗色主题、焦点态、图标按钮 `aria-label` 和 reduced-motion。
- **验收标准**：供应商和模型均无静默截断；主要操作无需猜测；375–1440px 无页面级横向滚动；构建通过。

## Task 11: 增量验证与回填

- [x] 读取并执行自动化测试标准；
- [x] 运行后端定向测试和 AI 插件完整测试；
- [x] 使用 Node `v20.19.0` 运行目标 Vue 文件 ESLint 与前端生产构建；
- [x] 执行 `xmllint`、`git diff --check`、敏感内容和 `gpt-3.5-turbo` 扫描；
- [x] 将命令、结果、警告、跳过项和服务清理情况追加到 `execution-log.md`；
- [x] 复查 Spec/Task/Test 状态并进入 review。

## Task 12: 归档与知识沉淀

- [x] 复核 Spec、Task、Test Spec、Execution Log 和长期记忆；
- [x] 确认架构决策与踩坑已沉淀到 `decisions.md` 第 16 条和 `pitfalls.md` 第 104 条；
- [x] 确认企业配置页视觉偏好已沉淀到 `preferences.md` 第 10 条；
- [x] 如实保留用户自行验收的最终 UI 微调，以及公网 DashScope/Flyway 实库环境跳过项；
- [x] 状态更新为 `done`，归档到 `code-copilot/changes/archive/2026-07-11-spring-ai-alibaba-provider-adapter/`。

...
- [x] **依赖基线**：根 POM 统一管理 Spring AI `1.1.2`、Spring AI Alibaba `1.1.2.3`、Extensions `1.1.2.3`，AI 插件只声明无版本依赖。
- [x] **适配器数据契约**：为 `ai_provider` 增加非空 `adapter_code`，既有数据全部回填 `openai_compatible`，通过新字典维护展示值。
- [x] **Provider Adapter SPI**：按 `adapterCode` 选择模型实现，未知值抛出 `BusinessException`，不依据 `providerType` 或 URL 猜测。
- [x] **OpenAI Compatible Adapter**：承接现有 `OpenAiApi/OpenAiChatModel` 行为，覆盖 OpenAI、DeepSeek、智谱、Moonshot、Ollama、自定义及旧阿里兼容模式。
- [x] **DashScope Native Adapter**：使用数据库配置动态创建 `DashScopeApi/DashScopeChatModel/DashScopeChatOptions`，支持同步、流式、推理内容和 Tool Calling 基础协议。
- [x] **调用层解耦**：`AiClientImpl` 和 `ChatClientCache` 只使用协议无关运行参数与 `ChatModel`，不再导入供应商 Options 类型。
- [x] **统一连接测试**：连接测试与正式调用复用同一 Adapter Registry；已保存供应商只传 ID，服务端读取真实凭据。
- [x] **缓存一致性**：供应商修改、删除成功后按 providerId 失效缓存；缓存键包含 adapter、模型及非敏感运行参数，不包含明文 API Key。
- [x] **密钥保护**：列表和详情返回前 4 位 + `****` + 后 4 位；短密钥全部掩码；提交未变化的脱敏值时保留数据库原值。
- [x] **管理端配置**：当前生效的 `provider-model.vue` 使用字典展示和选择适配器；新建默认为 `openai_compatible`，选择原生 DashScope 时使用原生 Base URL。
- [x] **回归验证**：覆盖适配器选择、参数映射、未知适配器失败关闭、密钥脱敏回写、缓存失效和既有 OpenAI Compatible 调用。
...
2. 供应商配置更新或删除成功后通过 after-commit 调度清理该租户/providerId 的所有客户端；事务回滚不得清缓存，无活动事务时在持久化成功返回后立即清理；若实际事务已激活但事务同步未启用，则失败关闭并拒绝提前清理；
...
| Task 3 | 完成 | `provider/adapter/*`、对应测试 | Registry、URL Policy、Compatible/Native Adapter 已完成，12 个测试通过；统一调用入口已由 Task 4/5 接通 |
| Task 4 | 完成 | `AiClientImpl`、`ChatClientCache`、`AiInvocationResolver`、`AiProviderCacheEvictionScheduler`、对应测试 | 调用层与供应商实现解耦；租户安全缓存键、after-commit 失效及事务同步异常失败关闭已验证 |
| Task 5 | 完成 | 供应商 DTO/VO、Service、Controller、Mapper XML、密钥与缓存支持类及测试 | 生命周期、严格 one-of 连接测试、密钥脱敏回写、SDK 异常安全化均已完成 |
| Task 6 | 完成 | `provider-model.vue`、`src/api/ai.js` | 活动供应商页面支持字典协议选择、受控 URL 联动、加密写请求和仅 ID 测试 |
| Task 7 | 完成 | AI 插件测试、装配、迁移和前端构建相关文件 | AI 最终 45 tests；AI/Admin package、Flyway 静态检查、Mapper XML 和前端构建通过；公网调用按条件跳过 |
| Task 8 | 完成 | 两份 AI 中枢文档、本变更四份文档、`decisions.md`、`pitfalls.md` | 已回填实际版本、阶段边界、回退前置条件和审查结论，进入 review |

## 12. 审查结论

- Spec 合规审查：PASS。功能点、Task、测试证据、范围边界和回退约束与实现一致；
- 独立读者测试：第三轮复审 PASS；前两轮发现的 7 项阻断/高风险语义均已修订；
- 代码质量审查：PASS。审查发现“实际事务已激活但事务同步未启用时可能立即清缓存”的边缘分支，已改为 fail-closed 并新增回归测试；
- 安全审查：PASS。API Key 仅经加密请求或数据库进入模型，响应脱敏，缓存键/日志/安全化异常不含密钥；
- 依赖审查：PASS。Spring AI 收敛到 `1.1.2`，Alibaba/Extensions 收敛到 `1.1.2.3`，未引入 DashScope Starter；
- 验证结论：AI 插件最终 45 tests 全过；AI/Admin package、Flyway 静态检查、Mapper XML 和 Node `20.19.0` 前端构建通过；未提供 `AI_DASHSCOPE_API_KEY`，公网真实调用按 Test Spec 跳过；既有 JVM/编译/Vite 警告保留在 `execution-log.md`；
- 当前阶段：`done`，已归档、未推送。

### 12.1 2026-07-11 独立复审

- Spec Compliance：PASS。重新从 `origin/main..HEAD` 检查 44 个变更文件，依赖、迁移、Adapter SPI、统一调用链、缓存、供应商生命周期、密钥边界和管理端协议均与 Spec 一致；
- Code Quality：PASS。未发现 Critical 或 Important 问题；Java 大括号、异常处理、租户缓存键、after-commit 失效、SDK 异常安全化及 Mapper XML 逻辑删除条件符合项目约定；
- 非阻断加固建议：非标准半迁移数据库若已存在定义不完整的 `adapter_code` 列，可在后续迁移中补列类型/default/nullability 校验；未来新增 Adapter 时，可进一步拆分连接测试的配置校验与网络调用异常边界；
- 独立验证：AI 插件 45 tests 全过、24 模块 reactor BUILD SUCCESS；依赖树收敛为 Spring AI `1.1.2` 和 DashScope Core `1.1.2.3`；Node `20.19.0` 前端 8485 modules 构建成功；Mapper XML、Flyway placeholder、Starter 缺失和 diff whitespace 检查通过；
- 保留边界：未提供公网 API Key 和隔离 dev 数据库，真实 DashScope 调用及 Flyway 实库执行仍按既有 Test Spec 跳过；供应商页面最后一轮间距、列表拆列和默认模型行内开关微调由用户自行验收，未把中断构建写成通过。
...
## 14. 2026-07-11 运行缺陷与管理端体验增量

### 14.1 运行缺陷

供应商连接测试实际返回 `400 invalid_request_error`，错误表明系统向仅支持
...
1. 删除所有 `gpt-3.5-turbo` 通用兜底，不允许根据连接协议或品牌猜测模型；
2. 已保存供应商的默认模型只从 `ai_model` 中读取 `status='0' AND is_default='1' AND del_flag='0'` 的记录；
3. 显式调用模型和 Agent 显式模型继续保持既有优先级，仅在需要供应商默认值时查询权威模型；
4. 未保存供应商连接测试继续使用请求中的 `defaultModel`，因为此时尚无 `ai_model` 记录；
5. 找不到可用默认模型时，在创建 `ChatModel` 和发起网络请求前抛出 `BusinessException("请为供应商设置默认模型")`；
6. SDK 异常响应仍向前端返回固定安全文案；服务端日志只增加脱敏后的 `httpStatus/errorCode`，禁止记录响应正文、
   API Key、Authorization Header、完整请求体或供应商错误 message。

### 14.2 管理端页面重构

活动页面 `forge-admin-ui/src/views/ai/provider-model.vue` 参考主流供应商配置控制台重构为克制的主从工作区：左侧供应商列表负责筛选、
分页和选择，右侧通过“基础配置 / 模型管理”页签展示所选供应商信息；窄屏自动改为上下布局。页面禁止堆叠无业务价值的
大标题、统计卡、英文装饰标签、渐变和引导步骤，遵循以下验收标准：

- 供应商分页使用响应式 `pageNum/pageSize/itemCount`，支持总条数、每页条数与快速跳页；
- 模型列表使用服务端分页，不再以 `pageSize=100` 截断；模型数量来自总条数而非当前页长度；
- 上传 `accept` 使用 `.png,.jpg,.jpeg,.svg,.webp`；供应商与模型弹窗使用 `max-width + 视口宽度`；
- API Key 编辑态明确提示“留空表示不修改”，前端不尝试还原密钥，后端继续保护脱敏占位值；
- 搜索统一为点击查询/回车查询，重置同时清空供应商选择与模型工作区；
- 点击供应商行只选择不反向取消，提供显式关闭按钮，并支持 Enter/Space 键盘选择；
- 两张表操作统一为语义化文字按钮；模型默认状态提供明确可操作入口；
- 连接测试结果同时展示图标、标题和说明，颜色不是唯一反馈；
- 浅色/暗色主题均有清晰边界，375/768/1024/1440 宽度无页面级横向溢出。

### 14.3 增量确认

- **确认时间**：2026-07-11（当前会话）
- **确认人**：用户明确要求修复默认模型、日志诊断并重新优化供应商配置页面

## 15. 归档记录（HARD-GATE）

- **归档时间**：2026-07-11
- **归档人**：code-copilot（用户明确执行归档）
- **归档路径**：`code-copilot/changes/archive/2026-07-11-spring-ai-alibaba-provider-adapter/`
- **知识沉淀**：`code-copilot/memory/decisions.md` 第 16 条、`code-copilot/memory/pitfalls.md` 第 104 条、`code-copilot/memory/preferences.md` 第 10 条
- **归档验收结论**：Spec Compliance、Code Quality、安全与依赖审查均通过；AI 插件 51 tests、Admin 35 模块装配、前端生产构建和三视口页面验证已有成功证据；最终 UI 微调按用户要求由用户自行验收，公网 DashScope 与 Flyway 实库验证继续保留为环境条件跳过项。

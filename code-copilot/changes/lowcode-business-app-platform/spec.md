...
- [ ] 应用中心进入深层配置时优先跳转或嵌入现有低代码管理页面，例如模型设计、应用搭建器、发布版本、代码预览。
- [ ] 现有低代码接口不因业务平台新增入口而改名或删除；如需业务化表达，通过新的 VO 或包装接口完成。
...
- 业务对象配置模型和布局时，采用跳转现有低代码页面、抽屉嵌入组件，还是在应用中心内做轻量向导。
- CRM 样板第一期是否必须至少配置一个真实可运行对象，例如客户管理。
- 应用包是否需要第一阶段新增独立表，还是先通过业务套件和初始化脚本表达。
- 对象级导入导出入口是否在第一阶段打通到动态 CRUD，还是只展示能力挂接。
- 嵌入应用白名单、SSO 和短期签名 URL 是否纳入第一阶段安全验收。

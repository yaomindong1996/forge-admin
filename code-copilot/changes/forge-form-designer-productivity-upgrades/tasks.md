...
- [x] 表单画布补齐“预览视口 / 设计宽度”控制，支持桌面、窄屏、弹窗、抽屉、移动端和自定义宽度，交互与列表设计器保持一致。
- [x] 修复表单画布缩放后拖拽镜像被压缩变形的问题，拖拽浮层按真实布局尺寸克隆后整体缩放。
- [x] 表单画布节点开始拖拽时自动收起右侧属性面板，释放中间画布操作空间。
- [x] 表单设计器外层高度和滚动结构对齐列表设计器，收起右侧后不再保留空白窄栏。
- [x] 修复栅格/表格内部投放区拖拽预览和最终 drop 不一致的问题，拖入“拖入字段或布局”块时优先命中真实内部槽位，避免误报“不支持”和外层灰色占位错位。
- [x] 修复从左侧组件库拖入栅格/表格内部投放区时，预览阶段读不到拖拽 payload 导致误报“不支持”的问题；左侧 dragstart 写入共享预览组件，画布 dragover 直接使用该预览对象做命中和高亮。
- [x] 修复表格布局运行态 `af-layout-table` / `af-layout-table-cell` 不响应“不显示边框”设计属性的问题，公共 AiForm 布局渲染会读取 `__designerStyle` 并隐藏真实表格和单元格边框。
